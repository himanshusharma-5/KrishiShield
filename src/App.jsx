/**
 * KrishiShield AI — Enhanced Production App
...
const CROPS = [
  { id: "wheat", name: "Wheat", emoji: "🌾", stage: "Flowering", health: 78, sector: "A-1 to A-4" },
  { id: "cotton", name: "Cotton", emoji: "🌿", stage: "Boll Formation", health: 91, sector: "B-2" },
  { id: "paddy", name: "Paddy", emoji: "🌱", stage: "Vegetative", health: 85, sector: "C-1" },
];

const MARKET_PRICES = [
  { crop: "Wheat", price: 2275, unit: "₹/qtl", change: +45, trend: "up", msp: 2275 },
  { crop: "Cotton", price: 7020, unit: "₹/qtl", change: -120, trend: "down", msp: 7020 },
  { crop: "Paddy", price: 2183, unit: "₹/qtl", change: +28, trend: "up", msp: 2183 },
  { crop: "Maize", price: 1962, unit: "₹/qtl", change: +15, trend: "up", msp: 1962 },
  { crop: "Soybean", price: 4600, unit: "₹/qtl", change: -85, trend: "down", msp: 4600 },
];
...
const ALERTS = [
  ...
  { id: 3, type: "info", title: "Optimal Spray Window", time: "8h ago", desc: "Low wind conditions (2.1 km/h) expected tomorrow 5–8 AM. Ideal for fungicide application.", read: true },
  ...
  { id: 5, type: "info", title: "MSP Update", time: "2d ago", desc: "Government announces revised MSP for Kharif 2025. Wheat: ₹2275/qtl (+₹150 from last year).", read: true },
];
...
async function getAIResponse(input) {
  ...
  if (text.includes("aphid") || text.includes("pest")) {
    return "🐛 Aphids: check the underside of leaves early morning. If colonies are active, start with neem-based spray and reassess after 24 hours. Avoid blanket spraying unless the infestation spreads.";
  }
  if (text.includes("wheat") || text.includes("harvest")) {
    return "🌾 Wheat: if the crop is mature and rain is possible, prioritize harvesting the driest blocks first. Keep harvested grain covered and avoid nitrogen application before wet weather.";
  }
  if (text.includes("rain") || text.includes("irrigation") || text.includes("water")) {
    return "🌧️ Rain/irrigation: delay irrigation if rain is likely or soil already feels moist. Spray only after leaves are dry and wind is calm.";
  }
  if (text.includes("price") || text.includes("market") || text.includes("mandi")) {
    return "₹ Market: compare today’s mandi price with MSP and your storage cost. If price is weak and storage is safe, holding for a few days can be better.";
  }
  return "✅ First scout the field, confirm the crop stage, and treat only the affected patch. Keep input costs low, avoid spraying during wind or rain risk, and check again after 24 hours.";
}
    ...
    <div className="px-4 py-6 space-y-5 max-w-2xl mx-auto">
      ...
      <div>
        ...
        <h1 className="text-3xl font-black leading-tight syne text-white">
          Sat Sri Akal,<br />{farmData?.farmerName || "Farmer Ji"} 🙏
        </h1>
      ...
      <div className="glass rounded-2xl p-5" style={{ background: "linear-gradient(135deg, rgba(5,46,22,0.6), rgba(14,23,18,0.8))" }}>
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <div className="flex items-end gap-2">
              <span className="text-5xl font-black syne text-white">{weather.current.temp == null ? "--" : weather.current.temp}°</span>
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            { label: "Temp", value: weather.current.temp == null ? "--" : `${weather.current.temp}°C`, icon: <Thermometer size={14} /> },
      ...
      <div>
        <div className="flex justify-between items-center mb-3">
          ...
          <button onClick={() => onNavigate("crophealth")} className="text-xs font-semibold" style={{ color: THEME.primary }}>See All →</button>
        </div>
        <div className="space-y-3">
          {CROPS.map(crop => (
            <button key={crop.id} onClick={() => onNavigate("crophealth")}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs mt-0.5" style={{ color: THEME.onSurfaceVariant }}>{crop.stage} · {farmData?.area || "Area not set"} · {crop.sector}</p>
      ...
      <div className="glass-sm rounded-2xl p-4">
        <div className="flex justify-between items-center mb-3">
          ...
          <button onClick={() => onNavigate("market")} className="text-xs font-semibold" style={{ color: THEME.secondary }}>Full Market →</button>
        </div>
...
function ForecastScreen({ onNavigate, location, weather }) {
  ...
  return (
    <div className="px-4 py-6 space-y-5 max-w-2xl mx-auto">
      <div>
        ...
        <div className="flex items-center gap-1.5 mt-1">
          ...
          <span className="text-xs" style={{ color: THEME.onSurfaceVariant }}>
            {location.name || "Fetching location..."} · {weather.current.updatedAt ? "Updated just now" : weather.current.source}
          </span>
        </div>
      </div>
          ...
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: THEME.error }}>
              {weather.current.temp == null
                ...
                : `${weather.current.condition} · ${weather.current.temp}°C · wind ${weather.current.wind}km/h`}
            </p>
          </div>
        </div>
      </motion.div>
        ...
        {weather.forecast.map((day, i) => (
          <button key={day.day} onClick={() => setSelectedDay(i)}
            ...
            <div className="text-center">
              <div className="text-sm font-black" style={{ color: day.alert ? THEME.error : "white" }}>{day.high}°</div>
              <div className="text-[10px]" style={{ color: THEME.onSurfaceVariant }}>{day.low}°</div>
            </div>
      ...
      <div className="glass rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold syne text-white">{selectedForecast.day} · Detailed View</h3>
          <WeatherIcon type={selectedForecast.icon} size={28} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          {[
            { label: "Condition", value: selectedForecast.condition, icon: <Cloud size={14} /> },
            { label: "Temperature", value: weather.current.temp == null ? "--" : `${weather.current.temp}°C`, icon: <Thermometer size={14} /> },
      ...
      <div className="glass rounded-2xl p-5" style={{ border: `1px solid ${THEME.primary}20` }}>
        ...
        <p className="text-sm leading-relaxed" style={{ color: THEME.onSurfaceVariant }}>
          {weather.current.wind != null && weather.current.wind > 18
            ? "⚠️ Wind is high right now. Delay spraying until calmer conditions."
            : weather.current.temp == null
              ? "Fetching weather guidance for your farm..."
              : "✅ Live weather is loaded for your location. Use the current temperature and wind before spraying or irrigating."}
        </p>
      </div>
    </div>
  );
  ...
  const [advisories, setAdvisories] = useState([
    {
      ...
      summary: "Wheat maturity at 94.2%. Storm approaching Thursday — harvest by Wednesday evening.",
      detail: "Satellite imagery confirms crop maturity at 94.2% (optimal: 92–96%). A high-intensity storm cell is approaching Thursday 2–8 AM with projected 45–65mm precipitation. Post-storm wet conditions will trigger Fusarium Head Blight within 48hrs, reducing grain quality by 15–20%. Immediate action: begin harvesting Monday evening, prioritize blocks A-1 and A-2 first.",
      actions: ["Schedule Harvester", "Mark Complete"],
    },
    {
      id: 2, crop: "Cotton", title: "Aphid Alert — Sector B-2",
    ...
    },
    {
      ...
      summary: "Wind < 5km/h tomorrow 5–9 AM. Apply Urea 46% @ 25kg/acre for tillering stage boost.",
      detail: "IMD forecast confirms wind speed stabilization (2–4 km/h) between 05:00–09:00 tomorrow. Soil pH: 6.8 (optimal for N uptake). Crop is at maximum tillering stage — nitrogen demand is highest. Recommendation: Urea 46% @ 25kg/acre + Zinc Sulphate @ 5kg/acre for micronutrient correction. Light dew forecast improves absorption by 12%.",
...
Give ONE specific new advisory I haven't covered yet. Format as JSON: {title, crop, urgency, summary, detail, actions[2]}`
    ...
    try {
      ...
      addNotification("✅ New AI advisory generated!", "success");
    } catch {
      addNotification("⚠️ Could not parse advisory. Try again.", "error");
    }
  };
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full"
                      ...
                      {adv.urgency === "critical" ? "🚨" : adv.urgency === "warning" ? "⚠️" : "✅"} {adv.urgency}
                    </span>
                      ...
                      <div className="mt-4 pt-4 border-t" style={{ borderColor: THEME.outline + "30" }}>
                        ...
                        <div className="flex gap-2 mt-4">
                          {adv.actions?.map(action => (
                            <button key={action} onClick={() => addNotification(`✅ ${action} noted!`, "success")}
...
function MarketScreen({ onNavigate, prices }) {
  ...
  const getInsight = async (crop) => {
    ...
    const resp = await callAI([{
      role: "user",
      content: `Current ${crop.crop} price: ${crop.price} ₹/qtl (MSP: ${crop.msp}, change: ${crop.change > 0 ? "+" : ""}${crop.change}). 
  ...
  return (
    <div className="px-4 py-6 space-y-5 max-w-2xl mx-auto">
      <div>
        ...
        <p className="text-xs mt-1" style={{ color: THEME.onSurfaceVariant }}>Tap any crop for AI price analysis · Updated 15 min ago</p>
      </div>
          ...
          <motion.button key={item.crop} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl"
                ...
                {item.crop === "Wheat" ? "🌾" : item.crop === "Cotton" ? "🌿" : item.crop === "Paddy" ? "🌱" : item.crop === "Maize" ? "🌽" : "🫘"}
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-bold text-white">{item.crop}</h3>
                    <p className="text-[10px] mt-0.5" style={{ color: THEME.onSurfaceVariant }}>MSP: ₹{item.msp}/qtl</p>
                  </div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 12 }}
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-[10px]" style={{ color: THEME.onSurfaceVariant }}>{selectedCrop.crop} · {selectedCrop.price} ₹/qtl</p>
              </div>
            </div>
            {loadingInsight ? (
              <div className="flex items-center gap-2" style={{ color: THEME.onSurfaceVariant }}>
                <Loader2 size={14} className="animate-spin" /><span className="text-sm">Analyzing market data…</span>
              </div>
            ) : (
          ...
          {[
            ...
            <div key={mandi.name} className="glass-sm rounded-xl p-3 flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-[10px]" style={{ color: THEME.onSurfaceVariant }}>{mandi.dist} · ⭐ {mandi.rating}</p>
              </div>
...
soilMoisture, nitrogenLevel, recommendation}. Use realistic values.`
    ...
    try {
      ...
      addNotification(`✅ ${selectedCrop.name} scan complete`, "success");
    } catch {
      setScanResult({
  ...
  return (
    <div className="px-4 py-6 space-y-5 max-w-2xl mx-auto">
      ...
      <div className="glass rounded-2xl p-5" style={{ background: "linear-gradient(135deg, rgba(5,46,22,0.5), rgba(14,23,18,0.7))" }}>
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-sm mt-1" style={{ color: THEME.onSurfaceVariant }}>{selectedCrop.stage} · {farmData?.area || "Area not set"} · {selectedCrop.sector}</p>
          </div>
          <div className="text-right">
      ...
      <button onClick={runAIScan} disabled={scanning}
        ...
        {scanning ? <><Loader2 size={18} className="animate-spin" />Running AI Scan…</> : <><Cpu size={18} />Run AI Crop Health Scan</>}
      </button>
        ...
        {scanResult && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            <div className="glass rounded-2xl p-5" style={{ border: `1px solid ${THEME.primary}25` }}>
              <h3 className="font-bold syne text-white mb-4">🤖 AI Scan Results</h3>
              ...
              <div className="space-y-3">
                ...
                {scanResult.issues.map((issue, i) => (
                  <div key={i} className="glass-sm rounded-xl p-4">
                    ...
                    <p className="text-xs font-semibold" style={{ color: THEME.primary }}>✅ {issue.treatment}</p>
                  </div>
                ))}
              </div>
            )}
...
function AIAssistantScreen({ onNavigate, farmData, location, weather }) {
  const [messages, setMessages] = useState([
    { role: "assistant", content: "Sat Sri Akal! 🙏 I'm your KrishiShield AI assistant. Ask me anything about your crops, weather, pests, soil, irrigation, or market prices. I'm here to help you get the best harvest!" }
  ]);
...
Current conditions: ${weather.current.temp}°C, ${weather.current.condition}, wind ${weather.current.wind} km/h.
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[11px]" style={{ color: THEME.primary }}>Offline AI advice · Always available</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="px-4 py-6 space-y-6 max-w-2xl mx-auto">
      ...
      <div className="space-y-2">
        {[
          ...
          { icon: <Globe size={18} />, label: "Language: English / ਪੰਜਾਬੀ / हिंदी", color: THEME.tertiary },
      ...
      <div className="text-center pb-2">
        <p className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: THEME.onSurfaceVariant }}>
          KrishiShield AI v2.0 · Built with ❤️ for Indian Farmers
        </p>
        <p className="text-[10px] mt-1" style={{ color: THEME.outline }}>
          AI advice runs offline · Location by browser GPS · Prices by Agmarknet
        </p>
      </div>
    </div>
  );
      ...
      content: (
        <div className="space-y-5">
          <div className="text-center mb-8">
            ...
            <div className="space-y-3">
              {["🤖 AI-powered crop health scanning", "🌦️ Hyper-local weather forecasting", "📈 Live mandi price updates", "🚨 Smart pest & disease alerts"].map(f => (
      ...
      content: (
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            { id: "paddy", name: "Paddy / Rice", emoji: "🌱", season: "Kharif" },
            { id: "maize", name: "Maize", emoji: "🌽", season: "Kharif" },
            { id: "soybean", name: "Soybean", emoji: "🫘", season: "Kharif" },
    ...
    <div className="min-h-screen px-4 py-8 flex flex-col max-w-md mx-auto">
      ...
      <div className="flex gap-3 mt-8">
        ...
        <button onClick={() => isLast ? onComplete(data) : setStep(s => s + 1)}
          ...
          {isLast ? "Start Farming 🌾" : "Continue"} <ArrowRight size={16} />
        </button>
      </div>
    </div>
  );
